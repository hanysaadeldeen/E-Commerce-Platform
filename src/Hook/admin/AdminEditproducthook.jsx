...
import { getSpecificProduct, updateOneProduct } from "../../reduxTool/ProdctSlice";
...
const AdminEditProducthook = (id) => {
    ...
    const EditedProduct = useSelector((state) => state.product.dataa)
    const getLoadingProduct = useSelector((state) => state.product.isLoading)

    const updatedProductf = useSelector((state) => state.product.updatedProduct)
    const statusUPdate = useSelector((state) => state.product.status)
    ...
    useEffect(() => {
        if (!getLoadingProduct) {
            setProductName(EditedProduct.title)
            setDescription(EditedProduct.description)
            setProductQuantity(EditedProduct.quantity)
            setProductPriceBefore(EditedProduct.price)
            setProductPriceBefore(EditedProduct.price)
            setProductPriceAfter(EditedProduct.priceAfterDiscount)
            getChoosedColor(EditedProduct.availableColors)
            setChoosenCat(EditedProduct.category);
            setchoosenBrand(EditedProduct.brand);
    ...
    // const { updatedProduct, error, status } = useSelector((state) => state.product)
    ...
    const getMainCategory = () => {
        if (choosenCat === "0") {
            dispatch(GetSubCategoryONCategory())

        } else {
            dispatch(GetSubCategoryONCategory(choosenCat))
    ...
    const submitEdit = async (e) => {
        ...
        if (images.length > 1 && productName !== "" && description !== "" && quantity !== "" && productPriceAfter !== "" &&
            ...
            toast.info("Pending...")
            await dispatch(updateOneProduct({ format, id }))
        }
        if (images.length <= 1 && productName === "" && description === "" && quantity === "" && productPriceAfter === "" &&
            productPriceBefore === "" && choosenCat === "" &&
            ChoosenColor.length === 0 && choosenBrand === ""
    ...
    useEffect(() => {
        if (updatedProductf.length !== 0 && statusUPdate !== 1) {
            toast.success("added  successfuvvvlly!", {
                theme: "colored",
            })
        } else if (error !== null) {
    ...
    return [submitEdit, getProductInfo, setProductName, setDescription,
