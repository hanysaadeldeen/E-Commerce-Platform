...
import ShopProductPagehook from "../Hook/ProductDeatils/ShopProductPagehook"
...
const ShopProductPage = () => {
    ...
    const [Allproduct, isLoading, paginationResult, error, allCategory,
        loadingCategory, allBrand, loadingbrand, getcategoryCheck,
        getBrandCheck, setOptionValue, setIsFilter, setOpen, isFilter,
        isOpen, setItemsPerPage, navigateToHome, btnrefIlter, btnref,
        getProductFilter, loadingProductF
    ] = ShopProductPagehook()
    ...
    const colors = [
        '#fff', '#D9E3F0', '#F47373',
        '#697689', '#37D67A', '#2CCCE4', '#000', '#dce775', '#ff8a65', '#ba68c8'
    ]
    ...
    return (
        <div className={` pt-20 relative `}>
            ...
            <div className="container ">
                {/* number products */}
                {
                    Allproduct && !isLoading && <div className="flex max-md:pb-2  max-lg:pb-4 lg:pb-7 items-center pl-2 text-lg font-semibold child:text-[#707070] gap-1">
                        <span className=" cursor-pointer"><span onClick={navigateToHome}> Home</span>  {`>`} show  <span className=" ml-1">{Allproduct.length} product </span> </span>
                    </div>
                }
                ...
                <div className="flex gap-4">
                        ...
                        <div className="w-full">
                            ...
                            <div className="max-lg:h-screen max-lg:overflow-y-auto  max-lg:pb-24 max-lg:pr-3">
                                ...
                                <div className="category mt-8">
                                    ...
                                    {
                                        !loadingCategory && allCategory && allCategory.map((item) => {
                                            return (
                                                <div className="control mb-1" key={item._id}>
                                                    <input type="checkbox" onChange={getcategoryCheck} value={item._id} id={item._id} />
                                ...
                                <div className="brand mt-8">
                                    <h1 className="font-extrabold text-sm uppercase border-b  border-[#787878] pb-2 mb-6">brand</h1>
                                    {
                                        !loadingbrand && allBrand && allBrand.map((item) => {
                                            return (
                                                <div className="control mb-1" key={item._id}>
                                                    <input type="checkbox" onChange={getBrandCheck} value={item._id} id={item._id} />
                                                    <label htmlFor={item._id}>{item.name}</label>
                                                </div>
                                            )
                                        })
                    ...
                    <div className="max-lg:w-full w-[calc(100%-210px)] " >
                        <div className="flex items-center justify-between lg:gap-5 lg:justify-end px-2 py-5 ">
                            ...
                            <div className="max-lg:hidden flex gap-2 items-center sort-by ">
                                ...
                                <Select
                                    ...
                                    className={"text-black  "}
                                    clearOnBlur={true}
                                    options={options1} onChange={(values) => setOptionValue((values[0].value))} />
                            </div>
                            {/* end sort for large screen  */}


                            {/* filter icon for smolle  screen */}
                        ...
                        <div>
                            ...
                            {
                                paginationResult && Allproduct.length > 0 &&
